...
const imagePaths = {
  ...
  avatar: 'public/og-assets/mock-avatar-256.jpg',
  ...
  return h('div', {
    ...
    : h('div', {
        style: {
          ...
          backgroundImage: 'linear-gradient(145deg, rgba(52,211,153,0.24), rgba(17,19,24,0.96))',
          color: '#d4d4d8', fontSize: distance === 0 ? 17 : 13, lineHeight: 1.35,
        },
      }, icon('post', distance === 0 ? 30 : 22, '#34d399'), h('div', { style: { display: 'flex', maxHeight: 96, overflow: 'hidden' } }, image?.title || '暂无封面')));
}
...
function creatorRow(data, images, avatarSize = 42) {
  return h('div', {
    style: { display: 'flex', alignItems: 'center', gap: 11, marginBottom: 18, color: '#d4d4d8', fontSize: visualLength(data.author) > 24 ? 14 : 17 },
  },
    ...
    style: {
      ...
      : h('div', {
          style: {
            ...
            backgroundImage: 'linear-gradient(145deg, rgba(192,132,252,0.26), rgba(17,19,24,0.96))',
            color: '#e4e4e7', fontSize: 24, lineHeight: 1.35,
          },
        }, icon('post', 44, data.accent), h('div', { style: { display: 'flex', maxHeight: 160, overflow: 'hidden' } }, cover?.title || data.title)))),
  h('div', {
    style: {
      display: 'flex',
      position: 'absolute',
...
export async function loadBaseImages(baseUrl) {
...
export async function prepareOgImage({
  ...
  const baseImages = await loadBaseImages(assetBaseUrl);
  const prepared = await prepareLiveResource(type, String(id), token, baseImages, apiBaseUrl);
  if (!prepared) return null;
  const fontData = await loadSubsetFont(collectFontText(prepared.data));
  return {
