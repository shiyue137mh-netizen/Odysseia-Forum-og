...
function sizedDiscordAttachmentUrl(value, size) {
...
async function loadRemoteImage(value, size) {
  const safeUrl = safeHttpUrl(value);
  const url = safeUrl ? sizedDiscordAttachmentUrl(safeUrl, size) : null;
...
async function prepareWorks(prefix, works, baseImages, emptyTitle) {
  ...
  const entries = await Promise.all(selected.map(async (work, index) => {
    ...
    return [key, await loadRemoteImage(work.image_url, 384).catch(() => ({ src: null, title: work.title }))];
  }));
...
async function prepareLiveThread(token, baseImages, id = THREAD_ID, apiBaseUrl) {
  ...
  const cover = metadata.image_url
    ? await loadRemoteImage(metadata.image_url, 768).catch(() => ({ src: null, title: metadata.title }))
...
function card(image, index, total) {
  ...
  return h('div', {
    ...
    : h('div', {
      ...
      }, icon('post', distance === 0 ? 30 : 22, '#d4d4d8'), h('div', { style: { display: 'flex', maxHeight: 102, overflow: 'hidden', paddingBottom: 6 } }, image?.title || '暂无封面')));
}
  ...
  h('div', {
    style: {
      ...
      : h('div', {
        ...
        }, icon('post', 44, '#d4d4d8'), h('div', { style: { display: 'flex', maxHeight: 166, overflow: 'hidden', paddingBottom: 6 } }, cover?.title || data.title)))),
  h('div', {
    style: {
      display: 'flex',
      position: 'absolute',
  ...
  ...(data.typeIcon === 'author' ? [
    h('div', { key: 'identity', style: { display: 'flex', alignItems: 'center', gap: 16, marginBottom: 22 } },
      ...
      h('div', { style: { display: 'flex', flexDirection: 'column', width: 300 } },
        h('div', { style: { display: 'flex', maxHeight: 110, overflow: 'hidden', paddingBottom: 6, fontSize: authorNameSize(data.author), lineHeight: 1.08, fontWeight: 700, textShadow: '0 4px 18px rgba(0,0,0,0.55)' } }, data.author),
    ...
    h('div', { key: 'latest', style: { display: 'flex', flexDirection: 'column' } },
      ...
      h('div', { style: { display: 'flex', maxHeight: 76, overflow: 'hidden', paddingBottom: 6, marginTop: 7, fontSize: Math.min(27, titleSize(data.title, 'collection')), lineHeight: 1.25, fontWeight: 700 } }, data.title)),
  ] : [
  h('div', { key: 'title',
    style: {
      display: 'flex',
